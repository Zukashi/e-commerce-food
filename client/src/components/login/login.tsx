...
export const Login = () => {
    ...
    const {register:registerRemember} = useForm();
            ...
            <div className='bg-form'>
                <div className='form-box'>
                    ...
                    <form className='register-form' onSubmit={handleSubmit(onSubmit)} autoComplete={'off'}>
                        {labels.map((label,i) => <span key={label[1]}><TextField type={i > 0 ? 'password' : 'text' }  {...register(label[1] as registerType)}  label={label[0]} variant="outlined"/><p className='error-message'>{errors[(label[1] as registerType)]?.message} </p></span>)}
